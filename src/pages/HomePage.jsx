  ...
  return (
    <div className="max-w-lg">
      ...
      <div className="flex gap-2 p-2 ">
        <input type="text" value={userInput} onChange={(e) => setUserInput(e.target.value)} placeholder="What to do" className="p-2 w-full border-cyan-400 border-solid border-2 rounded" />
        ...
        {todos.map((todo, index) => (
          <div key={index} className="flex justify-between p-2 bg-slate-300 rounded mb-4 items-center">
            ...
            <div className="flex gap-1 items-center">
              <input type="checkbox" name="completed" checked={todo.completed} onChange={() => handleMarkAsCompleted(todo.id)} />
              <label htmlFor="completed">Completed</label>
              <div></div>
              <button className="btn" onClick={() => handleRemoveTodo(todo.id)}>
                Remove
              </button>
            </div>
          </div>
